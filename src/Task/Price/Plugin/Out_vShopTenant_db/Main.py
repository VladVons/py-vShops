...
class TSql(TSqlBase):
    ...
    async def Product_Create(self, aDbl: TDbProductEx):
        ...
        @DASplitDbl
        async def SProduct_Attr(aDbl: TDbProductEx, _aMax: int, aIdx: int = 0, aLen: int = 0) -> TDbList:
            print('SProduct_Attr()', aIdx, aLen)

            Values = []
            for Rec in aDbl:
                for Key, Val in Rec.GetField('attr', {}).items():
        ...
        Log.Print(1, 'i', 'Product_Attr')
        await SProduct_Attr(aDbl, self.Conf.parts)
