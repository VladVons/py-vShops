...
class TParser(TParserBase):
    ...
    async def _GetData(self, aEan: str):
        ...
        Token = 'vrHAfeN5TLvLjX314tbYhiBeVd2Hbq1A9Lp7Hxao'

        Url = f'{self.UrlRoot}/api/search-suggestions'
        Payload = f'searchVal={aEan}&_token={Token}'
        ...
        async with aiohttp.ClientSession() as Session:
            async with Session.post(Url, data=Payload, headers=self.Headers) as Response:
                ...
                try:
                    ...
                    Log.Print(1, 'e', 'import new (Cookie + Payload token) from browser. Auto parser not implemented yet')
                    return

                if (Data):
                    Product = Data[0]
                    if (aEan == Product['barcode']):
                        Image = f"{self.UrlRoot}/storage/{Product['image']}"
                        Url = f'{self.UrlRoot}/{aEan}'
                        async with Session.get(Url) as Response:
                            Data = await Response.read()
                            ResParse = self.ParseScheme(Data)
                            Res = ResParse.get('data')
                            if (Res) and (TEan(aEan).Check()):
                                Res['url'] = Url
                                return Res
